...
class TestTableClassesCQLGeneration:
    ...
    def test_multi_clustering_column_metadata_vector_cassandra_table(self, mock_db_session: MockDBSession) -> None:
        ...
        mock_db_session.assert_last_equal(
            [
                (
                    "CREATE TABLE IF NOT EXISTS k.tn ( partition_id TEXT, row_id_0 INT, row_id_1 INT, vector VECTOR<FLOAT,128>, attributes_blob TEXT, metadata_s MAP<TEXT,TEXT>, PRIMARY KEY ( ( partition_id ) , row_id_0, row_id_1 )) WITH CLUSTERING ORDER BY (row_id_a ASC, row_id_b ASC) AND COMMENT = 'Descriptive comment here';",  # noqa: E501
                    tuple(),
                ),
                (
                    "CREATE CUSTOM INDEX IF NOT EXISTS idx_vector_tn ON k.tn (vector) USING 'org.apache.cassandra.index.sai.StorageAttachedIndex' WITH OPTIONS = { 'source_model': 'bert', 'similarity_function': 'dot_product' };",  # noqa: E501
                    tuple(),
                ),
                (
